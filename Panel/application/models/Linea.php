...
class Linea extends CI_Model
{
	...
	{
 		$query = $this->db->query("SELECT count(*) as row FROM linea")->row_array();
 		return $query['row'];
	}
  public function total()
    {
      return $this->db->query("SELECT count(*) as row FROM linea")->result_array();
    }
 ...
 // untuk config class pagination yg lainnya optional (suka2 lu.. :D )
    ...
    { 
      $this->db->select('*');
      $this->db->from('linea');
      $this->db->where('idLinea',$idc);
      $query = $this->db->get();
      return $query->row();

    }

    function obtenerFichaDelete($idLineaegoria)
    { 
      $this->db->select('*');
      $this->db->from('linea');
      $this->db->where('idLinea',$idLineaegoria);
      $query = $this->db->get();
      return $query->row();
  ...
  function modificaDatos($data,$idLinea)
  { 
    $this->db->where('idLinea', $idLinea);
    $this->db->update('linea', $data); 
  }

  function borrarDatos($idLinea)
  { 
    $this->db->where('idLinea', $idLinea);
