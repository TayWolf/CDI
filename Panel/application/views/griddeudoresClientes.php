                                ...
                                <div class="col-md-4">
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                            ...
                                            <input class="form-control" placeholder="Nombre del cliente" type="text" name="cliente" id="cliente" value="<?=$nombreCliente[0]['nombreCliente']?>" readonly required>
                                            <input type="hidden" placeholder="Nombre del cliente" type="text" name="idcliente" id="idCliente" value="<?=$nombreCliente[0]['idCliente']?>" readonly required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2">
                    ...
                    <div class="card">
                        <div class="body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                            ...
                                            <input class="form-control" name="cuentaPago" id="cuentaPago" required>
                                        </div>
                                    </div>
                                    <div class="form-group form-float">
                                        <div class="form-line">
                                ...
                                <div class="col-md-4">
                                    <p><b>Subtotal:</b> $<label style="font-weight: unset" id="subtotal">0</label></p></br>
                                    <p><b>Descuento:</b> $<label style="font-weight: unset" id="descuento">0</label></p></br>
                                    <p><b>IVA:</b> $<label style="font-weight: unset" id="iva">0</label></p></br>
                                    <p><b>Total:</b> $<label style="font-weight: unset" id="total">0</label></p></br>
...
<script>
    ...
    {
        $.ajax({
            url:"http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/getDatoCliente/"+idCliente,
            type: 'POST',
            // data: parametros,
            dataType: 'JSON',
...
<script>
    $(window).load(function ()
    {
                ...
                $.ajax({
                    url:"http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/getClientes/"+encodeURIComponent($("#cliente").val()),
                    dataType:"json",
                    success:function(ui)
                    {
                        console.log("hola");
                        ...
                        {
                            $("#idCliente").val(ui[i].idCliente);
                            getdatFact(ui[i].idCliente );
                            $("#cliente").val(ui[i].nombreCliente);
                            $("#RFC").val(ui[i].RFC);
                            $("#calle").val(ui[i].direccionCliente);
                            $("#codigoPostal").val(ui[i].CP);
                            $("#colonia").val(ui[i].Colonia);
                            $("#municipio").val(ui[i].municipio);
                            filtrar();
                        }
...
<script>
    ...
    function filtrar()
    {
        ...
        var parametros =
            {
            'idCliente': idCliente,
            'fechaInicial': fechaInicial, 'fechaFinal': fechaFinal, 'idCita': idCita, 'idPaciente': idPaciente
        };

        peticion= $.ajax({
            url:"http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/filtrarTabla",
            type: 'POST',
            data: parametros,
            dataType: 'JSON',
            success: function (data)
            {
                ...
                for (i; i < data.length; i++)
                {
                    ...
                    $("#tabla").append('<tr><td><input type="hidden" id="nombrEss'+i+'" value="'+data[i]['nombreEstudio']+'"><input class="checkboxAdeudo" type="checkbox" id="estudioSeleccionado'+i+'" name="estudioSeleccionado'+i+'" onChange="sumarAdeudo('+i+')"> ' +
                        ' <label for="estudioSeleccionado'+i+'"></label></td><td>' + data[i]['idCita'] + '<input type="hidden" id="cita'+(i + 1)+'" name="cita'+(i + 1)+'" value="'+data[i]['idCita']+'"></td><td>' + data[i]['fechaCita'] + '</td><td>' + data[i]['nombrePaci'] + '</td><td>' + data[i]['nombreEstudio'] + '</td><td>' + data[i]['indicacionesPaciente'] + '</td><td>$<data id="adeudo'+(i)+'"></data></td></tr>');
                    sacarPrecioEstudio(i,data[i]['idCita'],data[i]['tipoCitaa'],data[i]['tipoCitaa']);
                }
                $("#xxc").val(i);
            }
        });
    ...
    function filtroEstudio()
    {
        ...
        peticionFE= $.ajax({
            url:"http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/GetEsF",
            type: 'POST',
            data: parametros,
            dataType: 'JSON',
            success: function (data)
            {
                ...
                for (i; i < data.length; i++)
                {
                    $("#tabla").append('<tr><td><input type="hidden" id="nombrEss'+i+'" value="'+data[i]['nombreEstudio']+'"><input class="checkboxAdeudo" type="checkbox" id="estudioSeleccionado'+i+'" onChange="sumarAdeudo('+i+')">  <label for="estudioSeleccionado'+i+'"></label></td><td>' + (i + 1) + '<input type="hidden" id="cita'+(i + 1)+'" value="'+data[i]['idCita']+'"></td><td>' + data[i]['fechaCita'] + '</td><td>' + data[i]['nombrePaci'] + '</td><td>' + data[i]['nombreEstudio'] + '</td><td>' + data[i]['indicacionesPaciente'] + '</td><td>$<data id="adeudo'+(i)+'"></data></td></tr>');
...
<script>
    ...
    function sacarPrecioEstudio(i,idCita,tipC,idCli)
    {
        // var parametros={'idCita':idCita,'tipC': tipC};
        $.ajax({
            url:"http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/getPrecio/"+idCita+"/"+tipC+"/"+idCli,
            type: 'POST',
            //data: parametros,
            dataType: 'JSON',
            success: function (data)
    ...
    $("#formPagos").on("submit", function(e){
        ...
        $.ajax({
            ...
            complete: function()
            {
                ...
                $.ajax({
                    url:'http://localhost/CDI/Panel/index.php/CrudDeudoresClientes/insertaAdeudoCliente/'+$("#xxc").val()+"/"+encodeURIComponent('<?=$fecha?>')+"/"+$("#idCliente").val(),
                    type: "post",
                    dataType: "html",
