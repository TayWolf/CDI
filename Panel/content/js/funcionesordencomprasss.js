  ...
  function listaArticu()
  {
    ...
    if (idPr!="") {
     $.ajax({
            ...
            url : "http://localhost/CDI/Panel/index.php/Crudordencompra/traerMin/"+idPr,
            type: "POST",
            dataType: "JSON",
             ...
             if (data!="") {
              if(data.length > 0)
              {
                for(i=0; i<data.length; i++)
                {
                   // alert(data[i]["idUbicacion"]);
                  // alert("bien")
                   $("#listaOrden").append('<tr><td>'+data[i]["fechaEmitida"]+'</td><td><a href="#" onclick="abrirVentana('+data[i]["idCompra"]+')" >Ver</a></td><td><a href="#" onclick="abrirVentana('+data[i]["idCompra"]+')" >Editar</a></td></tr>');
             ...
             if (data!="") {
              if(data.length > 0)
              {
                for(i=0; i<data.length; i++)
                {
                   ...
                   $("#listaArEdit").append('<tr><td>'+data[i]["nombre"]+'</td><td><input type="checkbox" id="remeArtt'+data[i]["idArticulo"]+'" name="remeArtt'+data[i]["idArticulo"]+'" value="'+data[i]["idArticulo"]+'" onclick="ocultar('+data[i]["idArticulo"]+');" class="filled-in"><label for="remeArtt'+data[i]["idArticulo"]+'"></label></td><td><input type="text" name="canti'+data[i]["idArticulo"]+'" id="canti'+data[i]["idArticulo"]+'" value="'+data[i]["cantidad"]+'"></td></tr>');
                    // prueba(data[i]["idUbicacion"])
